...
#ifndef __MY_AP_FIXED_H__
#define __MY_AP_FIXED_H__
...
class my_ap_fixed{
	...
	struct Proxy{
		my_ap_fixed<_AP_W, _AP_I>* parent = nullptr;
		int hi, lo;
	...
	public:
		...
		// constructor
		//my_ap_fixed<_AP_W, _AP_I>(unsigned int u) {data = u;}
		my_ap_fixed<_AP_W, _AP_I>(float f) {
			int i_part;
			unsigned char i;
		...
		my_ap_fixed<_AP_W, _AP_I>(){ data = 0;}
		template <int _AP_W_2, int _AP_I_2>
		my_ap_fixed<_AP_W, _AP_I>(my_ap_fixed<_AP_W_2, _AP_I_2> op2){
	    ...
	    float to_float(){
	    	...
	    	if(sign_flag == 1) for(i=0; i<_AP_W; i++) bits_array[i] = (bits_array[i] == 1 ? 0 : 1);
